defmodule SqueezeWeb.Dashboard.CardsComponent do
  ...
  def weekly_distance(%{activity_summaries: summaries, current_user: user}) do
    ...
    distance =
      summaries
      |> Enum.filter(&(Timex.after?(&1.start_at_local, date) && &1.activity_type == :run))
      |> Enum.map(& &1.distance)
      |> Enum.sum()
  ...
  def last_week_distance(%{activity_summaries: summaries, current_user: user}) do
    ...
    distance =
      summaries
      |> Enum.filter(
        &(Timex.between?(&1.start_at_local, start_date, end_date) && &1.activity_type == :run)
      )
      |> Enum.map(& &1.distance)
      |> Enum.sum()
  ...
  def activity_name(%{current_user: user} = assigns) do
    ...
    cond do
      latest.distance > 0 ->
        "#{format_distance(latest.distance, user.user_prefs)} #{latest.activity_type}"

      latest.duration > 0 ->
        "#{format_duration(latest.duration)} #{latest.activity_type}"
