defmodule Squeeze.Strava.BulkImport do
  ...
  def import_from_file(user, filename) do
    ...
    folder = Path.join([:code.priv_dir(:squeeze), "static", "uploads", SlugGenerator.gen_slug()])
    File.mkdir(folder)

    # Copy over to working dir
    File.copy(filename, Path.join(folder, Path.basename(filename)))

    # Change directory into working dir
    File.cd(folder)

    # Export the zip
    System.cmd("unzip", [Path.basename(filename)])

    # Read the activities.csv
    # Create a stream to create activities, trackpoints, and laps
    File.stream!("activities.csv")
    ...
    |> Stream.map(fn row ->
      with {:ok, %{"Activity ID" => external_id} = csv_data} <- row,
        ...
        create_detailed_activity(user, csv_data)
      else
  ...
  defp create_detailed_activity(user, csv_data) do
    data = load_data(csv_data["Filename"])
