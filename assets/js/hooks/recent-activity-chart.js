...
export default {
  ...
  calcChartData() {
    ...
    return {
      datasets,
      labels: this.weeks.map((x) =>
        x.toLocaleString({ month: "short", day: "numeric" })
      ),
    };
  },
        ...
        plugins: {
          tooltip: {
            callbacks: {
              ...
              label: (context) => {
                const type = capitalize(context.dataset.label);

                if (this.field === "distance") {
                  const label = this.imperial ? "mi" : "km";
                  return `${type}: ${context.formattedValue} ${label}`;
                }
                if (this.field === "elevation_gain") {
                ...
                return `${type}: ${duration.toFormat("h 'hr' mm 'min'")}`;
              },
            },
          },
          legend: false,
